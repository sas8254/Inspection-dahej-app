...
import { jobTypesApi } from '@/api/dahej'
...
async function load() {
  ...
  error.value = ''
  try {
    items.value = await jobTypesApi.list()
  } catch (e) {
    error.value = 'Failed to load job types.'
  } finally {
    loading.value = false
  }
}
...
async function onAdd() {
  ...
  adding.value = true
  try {
    await jobTypesApi.create({ name: newName.value.trim() })
    newName.value = ''
    await load()
  } catch (e) {
    error.value = e.response?.data?.name?.[0] || 'Failed to add job type.'
  } finally {
    adding.value = false
  }
}
...
async function onSave() {
  ...
  try {
    await jobTypesApi.update(editing.value.id, { name: editName.value.trim() })
    editDialog.value = false
    await load()
  } catch (e) {
    error.value = e.response?.data?.name?.[0] || 'Failed to update job type.'
  } finally {
...
async function onDelete(item) {
  ...
  try {
    await jobTypesApi.remove(item.id)
    await load()
  } catch (e) {
    error.value = 'Failed to delete (in use by a job?).'
  }
}
...
<template>
  <v-card>
    ...
    <v-card-text>
      ...
      <v-form @submit.prevent="onAdd" class="d-flex ga-2 mb-4">
        <v-text-field v-model="newName" label="New job type name" density="compact" hide-details />
  ...
  <v-dialog v-model="editDialog" max-width="500">
    <v-card>
      <v-card-title>Edit job type</v-card-title>
      <v-card-text>
